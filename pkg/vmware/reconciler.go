...
func (r *Reconciler) reconcile() {
	...
	for namespace := range svcs {
		...
		currentServices, err := r.GimbalKubeClient.CoreV1().Services(namespace).List(metav1.ListOptions{LabelSelector: clusterLabelSelector})
		if err != nil {
		...
		for app, e := range eps {
			...
			currentk8sEndpoints, err := r.GimbalKubeClient.CoreV1().Endpoints(namespace).List(metav1.ListOptions{LabelSelector: clusterLabelSelector})
			if err != nil {
